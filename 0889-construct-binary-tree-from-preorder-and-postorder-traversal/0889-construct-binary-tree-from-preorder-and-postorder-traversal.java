...
class Solution {
    ...
    public TreeNode constructFromPrePost(int[] preorder, int[] postorder) {
        HashMap<Integer, Integer> map = new HashMap<>(); //for storing postorder value along with idx
    ...
    public TreeNode helper(int[] pre, int[] post, HashMap<Integer, Integer> map, int prelo, int prehi, int postlo, int posthi){
        ...
        if(prelo + 1 <= prehi){
            ...
            int te = sidx - postlo + 1;
            
            node.left = helper(pre, post, map, prelo + 1, prelo +  te, postlo, sidx);
            node.right = helper(pre, post, map, prelo + te + 1, prehi, sidx + 1, posthi - 1);
        }
